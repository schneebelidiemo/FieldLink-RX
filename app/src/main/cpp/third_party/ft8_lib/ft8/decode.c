...
int ftx_find_candidates(const ftx_waterfall_t* wf, int num_candidates, ftx_candidate_t heap[], int min_score)
{
    ...
    // The upstream fixed upper bound of 20 works for a normally aligned FT8
    // slot, but misses valid FT4 signals later in a complete 7.5 s capture
    // (including the upstream generator's centred reference waveform).
    // Search every start position for which the final data symbol still fits;
    // trailing Costas/ramp symbols may be outside the capture by design.
    int last_data_symbol = (wf->protocol == FTX_PROTOCOL_FT4)
        ? ((FT4_ND - 1) + 13)
        : ((FT8_ND - 1) + 14);
    int max_time_offset = wf->num_blocks - last_data_symbol;
